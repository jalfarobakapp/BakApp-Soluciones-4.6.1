...
#End Region
    ...
    FROM MAEEDO m
    INNER JOIN {Global_BaseBk}Zw_InterStock_Det d ON m.TIDO = d.Tido_OCC AND m.NUDO = d.Nudo_OCC
    ...
    FROM MAEEDO m
    INNER JOIN {Global_BaseBk}Zw_InterStock_Det d ON m.TIDO = d.Tido_NVV AND m.NUDO = d.Nudo_NVV
    INNER JOIN {Global_BaseBk}Zw_InterStock_Enc e ON e.Id_Enc = d.Id_Enc 
    INNER JOIN MAEEDO orig ON d.Tido = orig.TIDO AND d.Nudo = orig.NUDO
    WHERE e.Id_Enc = {IdEnc} 
    ...
    FROM MAEEDO m
    INNER JOIN {Global_BaseBk}Zw_InterStock_Det d ON m.TIDO = d.Tido_FCV AND m.NUDO = d.Nudo_FCV
    INNER JOIN {Global_BaseBk}Zw_InterStock_Enc e ON e.Id_Enc = d.Id_Enc 
    INNER JOIN MAEEDO orig ON d.Tido = orig.TIDO AND d.Nudo = orig.NUDO
    WHERE e.Id_Enc = {IdEnc} 
    ...
    FROM MAEEDO m
    INNER JOIN {Global_BaseBk}Zw_InterStock_Det d ON m.TIDO = d.Tido_FCC AND m.NUDO = d.Nudo_FCC
    INNER JOIN {Global_BaseBk}Zw_InterStock_Enc e ON e.Id_Enc = d.Id_Enc 
    INNER JOIN MAEEDO orig ON d.Tido = orig.TIDO AND d.Nudo = orig.NUDO
    WHERE e.Id_Enc = {IdEnc} 
    ...
    Public Sub SincronizarDocumentosCredito(IdEnc As String, Txt_Log As Object)
        Dim _Sql As New Class_SQL(Cadena_ConexionSQL_Server)
        Dim Consulta_sql_p As String = $"
    -- 1. ACTUALIZAR LAS NCC (Resta 10)
   
    UPDATE m
    SET HORAGRAB = orig.HORAGRAB + 6
    FROM MAEEDO m
    INNER JOIN {Global_BaseBk}Zw_InterStock_Det d ON m.TIDO = d.Tido_NCC AND m.NUDO = d.Nudo_NCC
    INNER JOIN {Global_BaseBk}Zw_InterStock_Enc e ON e.Id_Enc = d.Id_Enc 
    INNER JOIN MAEEDO orig ON d.Tido = orig.TIDO AND d.Nudo = orig.NUDO
    WHERE e.Id_Enc = {IdEnc} 
    ...
    -- 4. ACTUALIZAR LAS NCV (Resta 4)
    UPDATE m
    SET HORAGRAB = orig.HORAGRAB + 4
    FROM MAEEDO m
    INNER JOIN {Global_BaseBk}Zw_InterStock_Det d ON m.TIDO = d.Tido_NCV AND m.NUDO = d.Nudo_NCV
    INNER JOIN {Global_BaseBk}Zw_InterStock_Enc e ON e.Id_Enc = d.Id_Enc 
    INNER JOIN MAEEDO orig ON d.Tido = orig.TIDO AND d.Nudo = orig.NUDO
    WHERE e.Id_Enc = {IdEnc} 
    ...
    Public Function ObtenerDocsCredito() As DataTable
        ...
        Try
            Dim Consulta_sql As String = $"
            Select * from {Global_BaseBk}Zw_InterStock_Enc Where Procesar = 1 And Tido = 'NCV';"
    ...
    Public Function ContinuarProcesosInterrumpidos(Txt_Log As Object) As Mensajes
        ...
        WHERE Enc.Procesando = 1
          AND (ISNULL(Det.Idmaeedo_OCC, 0) = 0 
            ...
            OR ISNULL(Det.Idmaeedo_FCC, 0) = 0)"
