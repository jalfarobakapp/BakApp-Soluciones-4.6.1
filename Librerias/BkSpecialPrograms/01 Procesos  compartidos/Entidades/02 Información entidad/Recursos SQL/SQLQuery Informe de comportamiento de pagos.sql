...
-- PROMEDIO DE DIAS REAL DE PAGO
SELECT 
    ISNULL(ROUND(AVG(MAX_DIAS_REAL_PAGO),2),0) AS PROMEDIO_DIAS_REAL_PAGO
FROM #Paso_Informe_Ep
WHERE TIPO_DOC <> 'NCV' AND ESTADO_PAGO = 'C'

-- PROMEDIO DE DIAS REAL DE PAGO DE LAS ULTIMAS 3 FACTURAS PAGADAS
SELECT  
    ISNULL(AVG(MAX_DIAS_REAL_PAGO), 0) AS PROMEDIO_ULTIMAS_3_FACTURAS
FROM (
        SELECT TOP 3
            MAX_DIAS_REAL_PAGO
        FROM #Paso_Informe_Ep
