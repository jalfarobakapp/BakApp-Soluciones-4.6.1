...
SELECT D.KOPRCT,D.EMPRESA,E.TIDO,E.SUBTIDO,D.SULIDO,D.BOSULIDO,
      ...
      Into #INF_STOCK        
       ...
       WHERE D.PRCT=0 AND D.LILG IN ('SI','CR')  
             ...
             AND D.TIDO IN ('NVV','NVI')
             AND D.EMPRESA  =@Empresa 
             AND D.SULIDO   =@Sucursal
             AND D.BOSULIDO =@Bodega
             AND D.FEEMLI   <= @Fecha 
--GROUP BY D.KOPRCT,D.EMPRESA,E.TIDO,E.SUBTIDO,D.SULIDO,D.BOSULIDO,D.TIDOPA,
--         D.CAPRCO1,D.CAPRCO2,D.CAPRAD1,D.CAPRAD2,D.CAPREX1,D.CAPREX2
