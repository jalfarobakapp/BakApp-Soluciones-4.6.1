...
WHERE D.PRCT= 0  AND D.LILG IN ('SI','CR')  AND  D.ARPROD <> 'CIFRS'  AND D.EMPRESA  = '" & ModEmpresa & "'  AND D.FEEMLI<{d '2024-12-02'} GROUP BY D.KOPRCT 

UPDATE #PASOSALDO SET FIUD01 = 0 WHERE FIUD01 IS NULL 
UPDATE #PASOSALDO SET STPREPROV = COALESCE((SELECT SUM(D.CAPRCO1*FICO) FROM MAEDDO AS D WITH ( NOLOCK ) INNER JOIN MAEEDO WITH (NOLOCK) ON MAEEDO.IDMAEEDO = D.IDMAEEDO INNER JOIN TABTIDO WITH (NOLOCK) ON TABTIDO.TIDO = MAEEDO.TIDO WHERE #PASOSALDO.KOPRCT = D.KOPRCT  AND D.LILG IN ('SI','CR')  AND D.EMPRESA  = '" & ModEmpresa & "' AND D.FEEMLI<{d '2024-12-02'}  AND MAEEDO.TIDO IN ('GRP','GDD') AND MAEEDO.SUBTIDO IN ( 'PRE','CON')),0) 
UPDATE #PASOSALDO SET STPREPROV = STPREPROV - COALESCE((SELECT SUM(FCCPRES.CAPRCO1) FROM MAEDDO AS FCCPRES WITH ( NOLOCK ) INNER JOIN MAEEDO WITH (NOLOCK) ON MAEEDO.IDMAEEDO = FCCPRES.IDMAEEDO INNER JOIN MAEDDO AS GRPPAS WITH (NOLOCK) ON FCCPRES.IDRST = GRPPAS.IDMAEDDO AND FCCPRES.ARCHIRST = 'MAEDDO' AND GRPPAS.TIDO = 'GRP' WHERE #PASOSALDO.KOPRCT = FCCPRES.KOPRCT  AND FCCPRES.LILG IN ('SI','CR')  AND FCCPRES.EMPRESA  = '" & ModEmpresa & "' AND FCCPRES.FEEMLI<{d '2024-12-02'}  AND MAEEDO.TIDO IN ('BLC','FCC','FCT','FDC')),0) 
UPDATE #PASOSALDO SET STPRECLI = COALESCE((SELECT SUM(D.CAPRCO1*FICO*-1) FROM MAEDDO AS D WITH (NOLOCK) INNER JOIN MAEEDO WITH (NOLOCK) ON MAEEDO.IDMAEEDO = D.IDMAEEDO INNER JOIN TABTIDO WITH (NOLOCK) ON TABTIDO.TIDO = MAEEDO.TIDO WHERE #PASOSALDO.KOPRCT = D.KOPRCT  AND D.LILG IN ('SI','CR')  AND D.EMPRESA  = '" & ModEmpresa & "' AND D.FEEMLI<{d '2024-12-02'}  AND MAEEDO.TIDO IN ('GDP','GRD') AND MAEEDO.SUBTIDO IN ( 'PRE','CON')),0) 
UPDATE #PASOSALDO SET STPRECLI = STPRECLI - COALESCE((SELECT SUM(FCVPRES.CAPRCO1) FROM MAEDDO AS FCVPRES WITH ( NOLOCK ) INNER JOIN MAEEDO WITH (NOLOCK) ON MAEEDO.IDMAEEDO = FCVPRES.IDMAEEDO INNER JOIN MAEDDO AS GDPPAS WITH (NOLOCK) ON FCVPRES.IDRST = GDPPAS.IDMAEDDO AND FCVPRES.ARCHIRST = 'MAEDDO' AND GDPPAS.TIDO = 'GDP' WHERE #PASOSALDO.KOPRCT = FCVPRES.KOPRCT  AND FCVPRES.LILG IN ('SI','CR')  AND FCVPRES.EMPRESA  = '" & ModEmpresa & "' AND FCVPRES.FEEMLI<{d '2024-12-02'}  
...
SELECT D.IDMAEDDO,D.TIDO,D.NUDO,D.TIDOPA,D.KOPRCT,D.CAPRCO1,D.CAPRAD1,D.CAPREX1  
				...
				WHERE D.TIDO = 'GDI' AND D.TIDOPA = 'NVI' AND D.CAPRCO1 = D.CAPREX1 AND D.FEEMLI<{d '2024-12-02'} 
...
UPDATE #PASOSALDO  SET TRANSITO = COALESCE(( SELECT SUM(D.CAPRCO1-D.CAPRAD1)  FROM MAEDDO AS D WITH (NOLOCK)  INNER JOIN MAEEDO AS E WITH (NOLOCK) ON E.IDMAEEDO=D.IDMAEEDO AND E.EMPRESA='" & ModEmpresa & "'  WHERE D.KOPRCT=#PASOSALDO.KOPRCT AND D.TIDO IN ('GTI','GDI')  AND D.LILG IN ('SI','CR') AND  D.ARPROD <> 'CIFRS'  AND D.ARCHIRST NOT IN ('POTL','POTD','PODCE','PODCD')  AND D.FEEMLI<{d '2024-12-02'} AND NOT EXISTS (SELECT * FROM #GDINVI WITH (NOLOCK) WHERE #GDINVI.IDMAEDDO = D.IDMAEDDO) ) ,0) 
UPDATE #PASOSALDO  SET TRANSITO = TRANSITO - COALESCE(( SELECT SUM(D.CAPRCO1)  FROM MAEDDO AS D WITH (NOLOCK)  INNER JOIN MAEEDO AS E WITH (NOLOCK) ON E.IDMAEEDO=D.IDMAEEDO AND E.EMPRESA='" & ModEmpresa & "'  WHERE D.KOPRCT=#PASOSALDO.KOPRCT  AND D.TIDO = 'GRI'  AND D.TIDOPA IN ('GTI','GDI')  AND D.LILG IN ('SI','CR')  AND  D.ARPROD <> 'CIFRS'  AND D.FEEMLI<{d '2024-12-02'} ),0) 
UPDATE #PASOSALDO SET STEXISTINI = FIUD01 + TRANSITO - STPREPROV + STPRECLI 
UPDATE #PASOSALDO  SET ULT_FEDOCU = ( SELECT MAX(FEEMLI) FROM MAEDDO AS D WITH (NOLOCK)   WHERE D.KOPRCT = #PASOSALDO.KOPRCT AND D.EMPRESA = '" & ModEmpresa & "' AND D.FEEMLI <{d '2024-12-02'} )
...
UPDATE #PASOSALDO  SET PPPINI= ( SELECT TOP 1 (CASE WHEN D.ARPROD='COMOD' AND E.TIMODO='E' THEN D.PPPRPM*E.TAMODO ELSE D.PPPRPM END)+(CASE WHEN STEXISTINI<>0 THEN COALESCE(( SELECT SUM(MAEDCR.VALDCR)  FROM MAEDCR WITH ( NOLOCK )  INNER JOIN MAEDDO AS XD WITH (NOLOCK) ON XD.IDMAEDDO = MAEDCR.IDDDODCR  INNER JOIN MAEEDO AS XE WITH (NOLOCK) ON XE.IDMAEEDO=XD.IDMAEEDO AND XE.EMPRESA='" & ModEmpresa & "'  INNER JOIN MAEEDO AS XEBASE WITH (NOLOCK) ON XEBASE.IDMAEEDO=MAEDCR.IDMAEEDO AND XEBASE.EMPRESA='" & ModEmpresa & "'  WHERE XD.KOPRCT = D.KOPRCT  AND (XEBASE.FEEMDO> #PASOSALDO.ULT_FEDOCU OR (XEBASE.FEEMDO = #PASOSALDO.ULT_FEDOCU AND XEBASE.HORAGRAB > #PASOSALDO.ULT_HGDOCU))  AND XEBASE.FEEMDO <{d '2024-12-02'} ) ,0.0)/STEXISTINI ELSE 0 END)  FROM MAEDDO AS D WITH (NOLOCK)  INNER JOIN MAEEDO AS E WITH (NOLOCK) ON E.IDMAEEDO=D.IDMAEEDO AND E.EMPRESA='" & ModEmpresa & "'  WHERE D.KOPRCT=#PASOSALDO.KOPRCT  AND D.FEEMLI<{d '2024-12-02'} AND D.ARPROD NOT IN ('CIFRS')  ORDER BY D.FEEMLI DESC, E.HORAGRAB DESC, D.IDMAEDDO DESC, D.LILG DESC )
