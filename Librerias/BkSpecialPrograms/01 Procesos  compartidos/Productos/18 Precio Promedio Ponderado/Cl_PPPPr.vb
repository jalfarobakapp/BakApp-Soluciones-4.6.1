...
Public Class Cl_PPPPr
    ...
    Function Fx_RecalcularPPPxPR2(_Codigo As String,
        ...
        Try
            ...
            _SqlQuery += "Update MAEPREM Set PM = " & De_Num_a_Tx_01(_Pm, False, 5) & ",FEPM = '" & Format(Fepm, "yyyyMMdd") & "' Where EMPRESA = '" & Empresa & "' And KOPR = '" & _Codigo & "'" & vbCrLf &
                         ...
                         "Where CRIAS.IDMAEEDO = MAEDDO.IDMAEEDO And CRIAS.LILG = 'CR' And CRIAS.NULILG = MAEDDO.NULIDO And CRIAS.EMPRESA = '" & Empresa & "' Order By CRIAS.NULIDO DESC) " &
                         "Where MAEDDO.EMPRESA = '" & Empresa & "' And MAEDDO.KOPRCT = '" & _Codigo & "' And MAEDDO.LILG = 'GR'"
    ...
    Function Fx_DetalleDocumentos(_Codigo As String, _Fechinippp As DateTime) As DataTable
        ...
        Consulta_sql = "-- Crea Tabla de Paso para hacer el recalculo
                        ...
                        Cast(0 As Float) As SALDO,Cast(0 As Float) As V_SALDO,Cast(0 As Float) As NewPPPRPR2
					  	    INTO #DDO  
					            FROM MAEDDO AS D WITH ( NOLOCK )  
						             INNER JOIN MAEEDO AS E WITH ( NOLOCK ) ON E.IDMAEEDO=D.IDMAEEDO AND E.EMPRESA='" & Empresa & "'  
					    ...
					    WHERE 
						    ...
						    D.ARPROD <> 'COMOD' AND 
						    D.ARPROD <> 'CIFRS'  AND 
						    D.FEEMLI >= '" & Format(_Fechinippp, "yyyyMMdd") & "' AND
                            D.EMPRESA = '" & Empresa & "'
                        ...
                        INSERT INTO #DDO  
		                        ...
		                        FROM MAEDCR WITH ( NOLOCK )  
			                        INNER JOIN MAEDDO AS D WITH ( NOLOCK ) ON D.IDMAEDDO = MAEDCR.IDDDODCR  
				                        INNER JOIN MAEEDO AS E WITH ( NOLOCK ) ON E.IDMAEEDO=D.IDMAEEDO AND E.EMPRESA='" & Empresa & "'  
					                        INNER JOIN MAEEDO AS EBASE WITH ( NOLOCK ) ON EBASE.IDMAEEDO=MAEDCR.IDMAEEDO AND EBASE.EMPRESA='" & Empresa & "'  
						                        LEFT OUTER JOIN MAEEN WITH ( NOLOCK ) ON E.ENDO=MAEEN.KOEN AND E.SUENDO=MAEEN.SUEN  
                        WHERE 
	                        D.KOPRCT = '" & _Codigo & "'  AND 
	                        D.ARPROD <> 'COMOD' AND 
	                        D.ARPROD <> 'CIFRS' AND 
	                        EBASE.FEEMDO >= '" & Format(_Fechinippp, "yyyyMMdd") & "' AND
                            D.EMPRESA = '" & Empresa & "'
                        ORDER BY D.FEEMLI,E.HORAGRAB,D.IDMAEDDO,D.LILG 
...
WHERE D.PRCT= 0  AND D.LILG IN ('SI','CR')  AND  D.ARPROD <> 'CIFRS'  AND D.EMPRESA  = '" & Empresa & "' AND D.FEEMLI<@FECHADESDE GROUP BY D.KOPRCT 

UPDATE #PASOSALDO SET FIUD01 = 0 WHERE FIUD01 IS NULL 
UPDATE #PASOSALDO SET STPREPROV = COALESCE((SELECT SUM(D.CAPRCO1*FICO) FROM MAEDDO AS D WITH ( NOLOCK ) INNER JOIN MAEEDO WITH (NOLOCK) ON MAEEDO.IDMAEEDO = D.IDMAEEDO INNER JOIN TABTIDO WITH (NOLOCK) ON TABTIDO.TIDO = MAEEDO.TIDO WHERE #PASOSALDO.KOPRCT = D.KOPRCT  AND D.LILG IN ('SI','CR')  AND D.EMPRESA  = '" & Empresa & "' AND D.FEEMLI<@FECHADESDE  AND MAEEDO.TIDO IN ('GRP','GDD') AND MAEEDO.SUBTIDO IN ( 'PRE','CON')),0) 
UPDATE #PASOSALDO SET STPREPROV = STPREPROV - COALESCE((SELECT SUM(FCCPRES.CAPRCO1) FROM MAEDDO AS FCCPRES WITH ( NOLOCK ) INNER JOIN MAEEDO WITH (NOLOCK) ON MAEEDO.IDMAEEDO = FCCPRES.IDMAEEDO INNER JOIN MAEDDO AS GRPPAS WITH (NOLOCK) ON FCCPRES.IDRST = GRPPAS.IDMAEDDO AND FCCPRES.ARCHIRST = 'MAEDDO' AND GRPPAS.TIDO = 'GRP' WHERE #PASOSALDO.KOPRCT = FCCPRES.KOPRCT  AND FCCPRES.LILG IN ('SI','CR')  AND FCCPRES.EMPRESA  = '" & Empresa & "' AND FCCPRES.FEEMLI<@FECHADESDE  AND MAEEDO.TIDO IN ('BLC','FCC','FCT','FDC')),0) 
UPDATE #PASOSALDO SET STPRECLI = COALESCE((SELECT SUM(D.CAPRCO1*FICO*-1) FROM MAEDDO AS D WITH (NOLOCK) INNER JOIN MAEEDO WITH (NOLOCK) ON MAEEDO.IDMAEEDO = D.IDMAEEDO INNER JOIN TABTIDO WITH (NOLOCK) ON TABTIDO.TIDO = MAEEDO.TIDO WHERE #PASOSALDO.KOPRCT = D.KOPRCT  AND D.LILG IN ('SI','CR')  AND D.EMPRESA  = '" & Empresa & "' AND D.FEEMLI<@FECHADESDE  AND MAEEDO.TIDO IN ('GDP','GRD') AND MAEEDO.SUBTIDO IN ( 'PRE','CON')),0) 
UPDATE #PASOSALDO SET STPRECLI = STPRECLI - COALESCE((SELECT SUM(FCVPRES.CAPRCO1) FROM MAEDDO AS FCVPRES WITH ( NOLOCK ) INNER JOIN MAEEDO WITH (NOLOCK) ON MAEEDO.IDMAEEDO = FCVPRES.IDMAEEDO INNER JOIN MAEDDO AS GDPPAS WITH (NOLOCK) ON FCVPRES.IDRST = GDPPAS.IDMAEDDO AND FCVPRES.ARCHIRST = 'MAEDDO' AND GDPPAS.TIDO = 'GDP' WHERE #PASOSALDO.KOPRCT = FCVPRES.KOPRCT  AND FCVPRES.LILG IN ('SI','CR')  AND FCVPRES.EMPRESA  = '" & Empresa & "' AND FCVPRES.FEEMLI<@FECHADESDE  
AND MAEEDO.TIDO IN ('BLV','FCV','FDB')),0) 
INSERT INTO #PASOSALDO  SELECT M.KOPR,0,0,0,0,0,0,0,'','','',null,0  
				FROM MAEPR AS M WITH ( NOLOCK )  
				INNER JOIN MAEPREM AS E WITH (NOLOCK) ON E.KOPR=M.KOPR AND E.EMPRESA='" & Empresa & "'  
...
UPDATE #PASOSALDO  SET TRANSITO = COALESCE(( SELECT SUM(D.CAPRCO1-D.CAPRAD1)  FROM MAEDDO AS D WITH (NOLOCK)  INNER JOIN MAEEDO AS E WITH (NOLOCK) ON E.IDMAEEDO=D.IDMAEEDO AND E.EMPRESA='" & Empresa & "'  WHERE D.KOPRCT=#PASOSALDO.KOPRCT AND D.TIDO IN ('GTI','GDI')  AND D.LILG IN ('SI','CR') AND  D.ARPROD <> 'CIFRS'  AND D.ARCHIRST NOT IN ('POTL','POTD','PODCE','PODCD')  AND D.FEEMLI<@FECHADESDE AND NOT EXISTS (SELECT * FROM #GDINVI WITH (NOLOCK) WHERE #GDINVI.IDMAEDDO = D.IDMAEDDO) ) ,0) 
UPDATE #PASOSALDO  SET TRANSITO = TRANSITO - COALESCE(( SELECT SUM(D.CAPRCO1)  FROM MAEDDO AS D WITH (NOLOCK)  INNER JOIN MAEEDO AS E WITH (NOLOCK) ON E.IDMAEEDO=D.IDMAEEDO AND E.EMPRESA='" & Empresa & "'  WHERE D.KOPRCT=#PASOSALDO.KOPRCT  AND D.TIDO = 'GRI'  AND D.TIDOPA IN ('GTI','GDI')  AND D.LILG IN ('SI','CR')  AND  D.ARPROD <> 'CIFRS'  AND D.FEEMLI<@FECHADESDE ),0) 
UPDATE #PASOSALDO SET STEXISTINI = FIUD01 + TRANSITO - STPREPROV + STPRECLI 
UPDATE #PASOSALDO  SET ULT_FEDOCU = ( SELECT MAX(FEEMLI) FROM MAEDDO AS D WITH (NOLOCK)   WHERE D.KOPRCT = #PASOSALDO.KOPRCT AND D.EMPRESA = '" & Empresa & "' AND D.FEEMLI <@FECHADESDE )
UPDATE #PASOSALDO  SET ULT_HGDOCU = ( SELECT MAX(HORAGRAB) FROM MAEEDO AS E WITH (NOLOCK) INNER JOIN MAEDDO AS D ON D.IDMAEEDO = E.IDMAEEDO  WHERE D.KOPRCT = #PASOSALDO.KOPRCT AND D.EMPRESA = '" & Empresa & "' AND D.FEEMLI = #PASOSALDO.ULT_FEDOCU )
UPDATE #PASOSALDO  SET PPPINI= ( SELECT TOP 1 (CASE WHEN D.ARPROD='COMOD' AND E.TIMODO='E' THEN D.PPPRPM*E.TAMODO ELSE D.PPPRPM END)+(CASE WHEN STEXISTINI<>0 THEN COALESCE(( SELECT SUM(MAEDCR.VALDCR)  FROM MAEDCR WITH ( NOLOCK )  INNER JOIN MAEDDO AS XD WITH (NOLOCK) ON XD.IDMAEDDO = MAEDCR.IDDDODCR  INNER JOIN MAEEDO AS XE WITH (NOLOCK) ON XE.IDMAEEDO=XD.IDMAEEDO AND XE.EMPRESA='" & Empresa & "'  INNER JOIN MAEEDO AS XEBASE WITH (NOLOCK) ON XEBASE.IDMAEEDO=MAEDCR.IDMAEEDO AND XEBASE.EMPRESA='" & Empresa & "'  WHERE XD.KOPRCT = D.KOPRCT  AND (XEBASE.FEEMDO> #PASOSALDO.ULT_FEDOCU OR (XEBASE.FEEMDO = #PASOSALDO.ULT_FEDOCU AND XEBASE.HORAGRAB > #PASOSALDO.ULT_HGDOCU))  AND XEBASE.FEEMDO <@FECHADESDE ) ,0.0)/STEXISTINI ELSE 0 END)  FROM MAEDDO AS D WITH (NOLOCK)  INNER JOIN MAEEDO AS E WITH (NOLOCK) ON E.IDMAEEDO=D.IDMAEEDO AND E.EMPRESA='" & Empresa & "'  WHERE D.KOPRCT=#PASOSALDO.KOPRCT  AND D.FEEMLI<@FECHADESDE AND D.ARPROD NOT IN ('CIFRS')  ORDER BY D.FEEMLI DESC, E.HORAGRAB DESC, D.IDMAEDDO DESC, D.LILG DESC )
