...
SELECT TOP 1 EDO.*,
	   SUBSTRING(CONVERT(NVARCHAR, CONVERT(datetime, (EDO.HORAGRAB*1.0/3600)/24), 108),1,5) AS HORA,
	   CAST('' AS Varchar(10)) As NUMECOM,
       ISNULL(En.NOKOEN,'') AS 'RAZON',
       ISNULL(EDO.ENDOFI,'') AS 'ENT_FISICA',
       ISNULL((SELECT TOP 1 NOKOEN FROM MAEEN WHERE KOEN = EDO.ENDOFI),'') AS 'RAZON_FISICA',
       ISNULL(Tf.NOKOFU,'') AS 'FUNCIONARIO',
       --ISNULL((SELECT TOP 1 NOTIDO FROM TABTIDO TTD WHERE EDO.TIDO = TTD.TIDO),'') AS 'TIPODOCUMENTO',
       --ISNULL((SELECT TOP 1 NOTIDO FROM TABTIDO TTD WHERE EDO.TIDO = TTD.TIDO),'') AS 'NOTIDO',
	   ISNULL(Td.NOTIDO,'') AS 'TIPODOCUMENTO',
       ISNULL(Td.NOTIDO,'') AS 'NOTIDO',
       Cast('' As Varchar(10)) As LISACTIVA,
       CASE 
            WHEN EDO.TIDOELEC = 1 
                THEN 'Si' 
...
FROM MAEEDO EDO WITH ( NOLOCK )  
	Left Join MAEEN En On En.KOEN = EDO.ENDO And En.SUEN = EDO.SUENDO
		Left Join TABFU Tf On Tf.KOFU = EDO.KOFUDO
			Left Join TABTIDO Td On Td.TIDO = EDO.TIDO
				Left Join MAEENCON Con On EDO.RUTCONTACT = Con.RUTCONTACT
WHERE  EDO.IDMAEEDO=@Idmaeedo  
...
SELECT 
			  ...
			  ISNULL((SELECT TOP 1 IDMAEEDO FROM MAEDDO WITH (NOLOCK) WHERE IDMAEDDO = EDD.IDRST),'') AS IDMAEEDO_PA,
...
SELECT * FROM MAEEDOOB WITH (NOLOCK)
WHERE IDMAEEDO = @Idmaeedo        

SELECT TOP 1 CCOMPRD1.*,NUMECOM,PERIODO 
FROM CCOMPRD1  WITH (NOLOCK) 
	INNER JOIN CCOMPRD WITH (NOLOCK) ON CCOMPRD1.IDCOMPRD = CCOMPRD.IDCOMPRD 
		INNER JOIN CCOMPRE WITH (NOLOCK) ON CCOMPRD.IDCOMPRE = CCOMPRE.IDCOMPRE  
		WHERE CCOMPRD1.ARCHIRST='MAEEDO' AND CCOMPRD1.IDRST=@Idmaeedo 

SELECT * FROM MAEVEN WITH (NOLOCK) Where IDMAEEDO = @Idmaeedo
