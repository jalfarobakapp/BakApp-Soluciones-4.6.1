...
FROM MAEDDO Ddo WITH (NOLOCK)
    Left Join MAEEN WITH (NOLOCK) ON Ddo.ENDO = MAEEN.KOEN AND Ddo.SUENDO = MAEEN.SUEN
        Left Join MAEPREM Mpen WITH (NOLOCK) ON Mpen.KOPR = Ddo.KOPRCT AND Mpen.EMPRESA = @Empresa
            Left Join MAEDDO Fcc WITH (NOLOCK) ON Ddo.IDMAEDDO = Fcc.IDRST AND Fcc.TIDO = 'FCC'
                Left Join MAEPR Mp WITH (NOLOCK) On Mp.KOPR = Ddo.KOPRCT
...
UPDATE B
SET 
    Precio_Neto_UN = ISNULL(ROUND(B.VANELI / NULLIF(B.CAPRCO1,0),5),0) + ISNULL(Flete_Neto,0), 
    Precio_Bruto_UN = ISNULL(ROUND(B.VABRLI / NULLIF(B.CAPRCO1,0),5),0) + ISNULL(Flete_Bruto,0)
FROM #Tbl_Paso2 B
...
CROSS APPLY (
    SELECT TOP 1
        ...
        ISNULL((
                SELECT ROUND(SUM(CR.VALDCR) * 1.0 / NULLIF(Ddo.CAPRCO1,0),5)
                FROM MAEDCR CR WITH (NOLOCK)
                WHERE CR.IDDDODCR = Ddo.IDMAEDDO
        ),0) AS 'Precio_Neto_UN_Ant',
        ISNULL(ROUND(Ddo.VABRLI / NULLIF(Ddo.CAPRCO1,0),5),0) +
        CASE
            WHEN ISNULL(MpAnt.POIVPR,0) = 0 THEN
                ISNULL((
                        SELECT ROUND(SUM(CR.VALDCR) * 1.0 / NULLIF(Ddo.CAPRCO1,0),5)
                        FROM MAEDCR CR WITH (NOLOCK)
                        WHERE CR.IDDDODCR = Ddo.IDMAEDDO
            ...
            ELSE ROUND(
                    ISNULL((
                            SELECT SUM(CR.VALDCR) * 1.0 / NULLIF(Ddo.CAPRCO1,0)
                            FROM MAEDCR CR WITH (NOLOCK)
                            WHERE CR.IDDDODCR = Ddo.IDMAEDDO
                    ),0) * (1 + (ISNULL(MpAnt.POIVPR,0) / 100.0)),
                0)
