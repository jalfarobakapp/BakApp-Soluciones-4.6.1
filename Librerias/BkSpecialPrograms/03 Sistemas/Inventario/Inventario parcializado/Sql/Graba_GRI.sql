...
If @Nudo = '' Begin
   SELECT TOP 1 GRI FROM CONFIEST WITH ( NOLOCK )  WHERE EMPRESA=@Empresa AND MODALIDAD=@Modalidad
   -- Colocamos el numero del nuevo documento
   SET @Nudo = (SELECT TOP 1 GRI FROM CONFIEST WITH ( NOLOCK )  WHERE EMPRESA=@Empresa AND MODALIDAD=@Modalidad)
    ...
    IF @Nudo is not null and rtrim(ltrim(@Nudo)) <> '' and @Nudo <> '0000000000' Begin
         ...
         UPDATE CONFIEST SET GRI=@NvoNumero1 WHERE MODALIDAD=@Modalidad AND EMPRESA=@Empresa  
    END

    IF @Nudo is null or rtrim(ltrim(@Nudo)) = '' Begin
      SET @Nudo = (SELECT TOP 1 GRI FROM CONFIEST WITH ( NOLOCK )  WHERE EMPRESA=@Empresa AND MODALIDAD='  ')
    End
  
    If @Nudo = '0000000000' Begin
      Set @Nudo = (SELECT COALESCE(MAX(NUDO),'0000000000') FROM MAEEDO WITH (NOLOCK) Where TIDO = 'GRI')
...
--     SELECT @Sudo   = @Sudo   + replicate(' ' ,  3 - len(@Sudo) )
    ...
    begin
      ...
      INSERT INTO MAEEDOOB ( IDMAEEDO,OBDO,OCDO,CPDO,DIENDESP,MOTIVO,
                 ...
                 DELETE x_maeedo where TIDO = 'GRI' and NUDO = @NumeroTrans 
                 DELETE x_maeddo where TIDO = 'GRI' and NUDO = @NumeroTrans 
...
Set @Obs = @Idmaeedo
Set @Obs = @Obs + ' - Guia '+ @Nudo + ' Guardada correctamente,' --+ 'Idmaeedo = '+@Idmaeedo
...
BEGIN CATCH
   ...
   RAISERROR('Se ha producido un error al grabar GRI: %d', 16, 1,@Nudo)
END CATCH
...
FROM MAEDDO 
WHERE TIDO = 'GRI' AND NUDO = 'INV0000082'
