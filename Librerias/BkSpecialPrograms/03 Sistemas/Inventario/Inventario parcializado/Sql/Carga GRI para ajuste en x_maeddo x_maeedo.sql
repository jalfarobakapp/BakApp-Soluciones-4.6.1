...
DELETE x_maeedo where TIDO = 'GRI' and NUDO = @NroGRI 
DELETE x_maeddo where TIDO = 'GRI' and NUDO = @NroGRI 

Set @Endo    = (SELECT RUT FROM CONFIGP WHERE EMPRESA=@Empresa )


Set @Caprco     = (Select Isnull(Sum(CantidadUd1),0) AS CAPRCO
                   FROM  dbo.Zw_TmpInv_InvParcial
                   Where Levantado = 0 and FechaInv = @FechaInv
                   and CantidadUd1 > 0)

Set @Vanedo     = (Select Isnull(Sum((CantidadUd1*(CostoUnitUd1))),0) AS VANELI
                   FROM  dbo.Zw_TmpInv_InvParcial
                   Where Levantado = 0 and FechaInv = @FechaInv
                   and CantidadUd1 > 0)
...
IF @Vabrdo > 0 Begin
 ...
 SELECT     'GRI' AS TIDO,
           ...
           CodigoPr AS KOPRCT,
           1 AS UDTRPR,
           CantidadUd1 AS CAPRCO1,
           CantidadUd2 AS CAPRCO2,
           ...
           0 AS VADNELI,
           19 AS POIVLI,
    (CantidadUd1*(CostoUnitUd1))*0.19 AS VAIVLI,
           (CantidadUd1*(CostoUnitUd1)) AS VANELI,
 ROUND((CantidadUd1*(CostoUnitUd1))*1.19,0) AS VABRLI,
           1 AS LINCONDESP
                   --   StockActual, FechaInv, HoraInv, CantidadUd1, CantidadUd2, IDMAEDDO, Levantado, Orden
 FROM  dbo.Zw_TmpInv_InvParcial
 Where Levantado = 0 and FechaInv = @FechaInv
 and CantidadUd1 > 0
End

Update dbo.Zw_TmpInv_InvParcial
 Set Levantado = 1
 Where Levantado = 0 and FechaInv = @FechaInv
 and CantidadUd1 > 0
