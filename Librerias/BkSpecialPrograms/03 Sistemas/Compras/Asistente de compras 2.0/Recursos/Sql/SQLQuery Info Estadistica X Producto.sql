...
Select KOPRCT as 'Codigo',
       ...
       CASE WHEN TIDO IN ('NCE','NCV','NCX','NCZ','NEV','GRI') THEN - 1 * CAPRCO1
	   ...
	   Round(Sum( CASE 
                       WHEN TIDO IN ('NCE','NCV','NCX','NCZ','NEV','GRI') THEN - 1 * VANELI 
                       WHEN TIDO IN ('GDV','GDP') THEN CASE When CAPRCO1-CAPREX1 = 0 Then 0 Else CAPRCO1-CAPREX1 * (VANELI/CAPRCO1) End 
                       ELSE VANELI END),0)  As 'Valor',
       Round(Sum( CASE 
                       WHEN TIDO IN ('NCE','NCV','NCX','NCZ','NEV','GRI') THEN - 1 * VABRLI 
                       WHEN TIDO IN ('GDV','GDP') THEN CASE When CAPRCO1-CAPREX1 = 0 Then 0 Else CAPRCO1-CAPREX1 * (VABRLI/CAPRCO1) End 
       ...
       Where 
       KOPRCT In #Codigo#  
       --And (TIDO IN ('FCV', 'FDB', 'FDV', 'FDX', 'FDZ', 'FEV', 'FVL', 'FVT', 'FVX', 'FVZ','FEE', 'BLV','GDV','GDP','NCE', 'NCV', 'NCX', 'NCZ', 'NEV') 
       --     Or 
       --     TIDO In ('GDI','GRI') And ARCHIRST = 'POTD')
       #Filtro_Documentos# 
       And FEEMLI BETWEEN @FechaInicio AND @FechaTermino
...
Select KOPRCT as 'Codigo',
	   ...
	   Round(Sum( CASE WHEN TIDO IN ('NCE','NCV','NCX','NCZ','NEV') THEN - 1 * VANELI 
       WHEN TIDO IN ('GDV','GDP') THEN CASE When CAPRCO1-CAPREX1 = 0 Then 0 Else CAPRCO1-CAPREX1 * (VANELI/CAPRCO1) End 
            ELSE VANELI END),0)  As 'Valor',
       Round(Sum( CASE WHEN TIDO IN ('NCE','NCV','NCX','NCZ','NEV') THEN - 1 * VABRLI 
       WHEN TIDO IN ('GDV','GDP') THEN CASE When CAPRCO1-CAPREX1 = 0 Then 0 Else CAPRCO1-CAPREX1 * (VABRLI/CAPRCO1) End 
            ELSE VABRLI END),0)  As 'ValorB'     
            
       Into #Vts_EntExcluidas#
       From MAEDDO 
       Where 
       KOPRCT In #Codigo# And 
       TIDO IN ('FCV', 'FDB', 'FDV', 'FDX', 'FDZ', 'FEV', 'FVL', 'FVT', 'FVX', 'FVZ','FEE', 'BLV',
	            'GDV','GDP','NCE', 'NCV', 'NCX', 'NCZ', 'NEV') And
