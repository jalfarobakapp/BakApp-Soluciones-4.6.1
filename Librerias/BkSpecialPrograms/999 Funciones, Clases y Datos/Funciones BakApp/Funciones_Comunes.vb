...
#End Region
    ...
    ' 1. Mantener la lógica existente para separar la parte entera y la decimal (variable "entero" y "dec").
    ' 2. Al finalizar la conversión de la parte entera (variable "palabras"), convertir la parte decimal ("dec") a palabras:
    '    a. Normalizar "dec" a dos dígitos (p. ej. "5" -> "50" o "05" según el comportamiento actual: si tiene un dígito se añade "0" a la derecha).
    '    b. Tomar los dos primeros dígitos y convertirlos a palabras en español (0..99) aplicando reglas:
    '       - 0..15: mapeo directo (cero, uno, ..., quince)
    '       - 16..19: "dieci" + unidad (dieciseis)
    '       - 20..29: "veinte" o "veinti" + unidad
    '       - >=30: decena ("treinta", "cuarenta", ...) y si unidad > 0 -> "decena y unidad"
    '    c. Añadir sufijo "centavos" (o similar) al texto decimal convertido.
    ' 3. Devolver la cadena final: "<palabras> con <decimales_palabras>" si hay decimales, o solo "<palabras>" si no.
    ' 4. Implementar la conversión decimal en una función auxiliar privada `DecimalesEnPalabras`.
    ' 5. Mantener compatibilidad con la lógica previa (negativos, límites), sin alterar el resto del módulo.
    '
    ' IMPLEMENTACIÓN: se reemplaza la función `Letras` original y se añade la función auxiliar `DecimalesEnPalabras`.
    Public Function New_Letras(numero As String) As String
        ...
        If Val(numero) <= 999999999 Then
            ...
            '**********Une la parte entera y la parte decimal*************
            If dec <> "" Then
                Dim decPalabras As String = DecimalesEnPalabras(dec)
                New_Letras = palabras & "con " & decPalabras & " "
            Else
                New_Letras = palabras
            End If
        Else
    ...
    ' Función auxiliar para convertir los dos primeros dígitos decimales a palabras (0..99)
    Private Function DecimalesEnPalabras(dec As String) As String

        If String.IsNullOrEmpty(dec) Then Return String.Empty

        ' Normalizar y tomar los dos primeros dígitos
        If dec.Length >= 2 Then
            dec = dec.Substring(0, 2)
        Else
            dec = dec.PadRight(2, "0"c)
        End If

        Dim valor As Integer
        If Not Integer.TryParse(dec, valor) Then
            Return dec & " centavos"
        End If

        Dim unidades() As String = {"cero", "uno", "dos", "tres", "cuatro", "cinco", "seis", "siete", "ocho", "nueve", "diez", "once", "doce", "trece", "catorce", "quince"}
        Dim decena As Integer = valor \ 10
        Dim unidad As Integer = valor Mod 10
        ...
        Select Case valor
            ...
            Case 16 To 19
                resultado = "dieci" & unidades(valor - 10)
            Case 20
                resultado = "veinte"
            Case 21 To 29
