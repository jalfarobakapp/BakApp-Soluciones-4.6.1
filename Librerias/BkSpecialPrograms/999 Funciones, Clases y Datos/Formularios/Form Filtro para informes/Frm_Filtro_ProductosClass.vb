...
Public Class Frm_Filtro_ProductosClass
    ...
    Dim CodInforme As String
    Dim Para_Seleccion_Tablas As Boolean = False
    Dim Tabla_Seleccionada As String
    ...
    Private Sub Frm_06_AsisCompra_Filtros_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Para_Seleccion_Tablas Then
            Rev_ChkBtn(BtnClasLibre, CBool(_Sql.Fx_Trae_Dato(_Global_BaseBk & "Zw_TblFiltro_InfxUs", "MarcarTodo",
                                                     "Funcionario = '" & FUNCIONARIO &
                                                     "' And Informe = '" & CodInforme & "' And Tabla = 'CLALIBPR'")))

            Rev_ChkBtn(BtnFamilias, CBool(_Sql.Fx_Trae_Dato(_Global_BaseBk & "Zw_TblFiltro_InfxUs", "MarcarTodo",
                                                     "Funcionario = '" & FUNCIONARIO &
                                                     "' And Informe = '" & CodInforme & "' And Tabla = 'FMPR'")))

            Rev_ChkBtn(BtnMarca, CBool(_Sql.Fx_Trae_Dato(_Global_BaseBk & "Zw_TblFiltro_InfxUs", "MarcarTodo",
                                                     "Funcionario = '" & FUNCIONARIO &
                                                     "' And Informe = '" & CodInforme & "' And Tabla = 'MRPR'")))

            Rev_ChkBtn(BtnRubros, CBool(_Sql.Fx_Trae_Dato(_Global_BaseBk & "Zw_TblFiltro_InfxUs", "MarcarTodo",
                                                     "Funcionario = '" & FUNCIONARIO &
                                                     "' And Informe = '" & CodInforme & "' And Tabla = 'RUPR'")))

            Rev_ChkBtn(BtnZona, CBool(_Sql.Fx_Trae_Dato(_Global_BaseBk & "Zw_TblFiltro_InfxUs", "MarcarTodo",
                                                     "Funcionario = '" & FUNCIONARIO &
    ...
    Function CargarFiltro(ByVal TipoDeFiltro As String,
                     ByVal CdInforme As String,
                     ByVal InfTextoEnPantalla As String,
                     ByVal FormularioPadre As Object,
                     Optional ByVal Incluye_Sin_Clasificacion As Boolean = True)

        Dim Fm As New Frm_FiltroInf_Mt
        Fm.Pro_TipoDeFiltro = TipoDeFiltro
        Fm.Pro_CodInforme = CdInforme
        Fm.CargarDatos(Incluye_Sin_Clasificacion)
        Fm.Text = InfTextoEnPantalla
        Fm.ShowDialog(FormularioPadre)
        Return Fm.Pro_Chekeado
